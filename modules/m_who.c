...
struct Message who_msgtab = {
	...
	.handlers[CLIENT_HANDLER] =		{ .handler = m_who, .min_para = 2 },
	.handlers[RCLIENT_HANDLER] =		{ mm_ignore },
	.handlers[SERVER_HANDLER] =		{ mm_ignore },
	.handlers[ENCAP_HANDLER] =		{ mm_ignore },
	.handlers[OPER_HANDLER] =		{ .handler = m_who, .min_para = 2 },
};
...
//	struct Channel *chptr = NULL;
	...
	if((*(mask + 1) == '\0') && (*mask == '*'))
	{
		...
		sendto_one_numeric(source_p, s_RPL(RPL_ENDOFWHO), "*");
		return 0;
	}
	...
	if(IsOperSpy(source_p) && *mask == '!')
	{
		...
		if(EmptyString(mask))
		{
			sendto_one_numeric(source_p, s_RPL(RPL_ENDOFWHO), parv[1]);
			return 0;
		}
	}
	...
	if(IsChannelName(mask))
	{
		...
		if(chptr != NULL)
		{
			...
			if(IsMember(source_p, chptr) || operspy)
				do_who_on_channel(source_p, chptr, server_oper, true);
			else if(!SecretChannel(chptr))
				do_who_on_channel(source_p, chptr, server_oper, false);
			ClearCork(source_p);
		}
		sendto_one_numeric(source_p, s_RPL(RPL_ENDOFWHO), parv[1] + operspy);
		return 0;
	}
	...
	if(((target_p = find_named_person(mask)) != NULL) && (!server_oper || IsOper(target_p)))
	{
		...
		sendto_one_numeric(source_p, s_RPL(RPL_ENDOFWHO), mask);
		return 0;
	}
	...
	if(!IsOper(source_p))
	{
		...
		{
			sendto_one_numeric(source_p, s_RPL(RPL_LOAD2HI), "WHO");
			sendto_one_numeric(source_p, s_RPL(RPL_ENDOFWHO), "*");
			return 0;
		}
		else
...
who_global(struct Client *source_p, const char *mask, int server_oper, int operspy)
{
	...
	if(maxmatches <= 0)
		sendto_one_numeric(source_p, s_RPL(ERR_TOOMANYMATCHES), "WHO");
}
...
do_who(struct Client *source_p, struct Client *target_p, const char *chname, const char *op_flags)
{
	...
	sendto_one_numeric(source_p, s_RPL(RPL_WHOREPLY),
		   (chname) ? (chname) : "*",
		   target_p->username,
