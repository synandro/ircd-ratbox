...
rb_ssl_tryaccept(rb_fde_t *F, void *data)
{
	...
	ret = do_ssl_handshake(F, rb_ssl_tryaccept, data);
...
rb_ssl_start_accepted(rb_fde_t *new_F, ACCB * cb, void *data, int timeout)
{
	...
	gnutls_credentials_set(*ssl, GNUTLS_CRD_CERTIFICATE, new_F->sctx->x509);
	gnutls_dh_set_prime_bits(*ssl, 1024);
	gnutls_transport_set_ptr(*ssl, (gnutls_transport_ptr_t) (long int)new_F->fd);
	if(do_ssl_handshake(new_F, rb_ssl_tryaccept, data))
	{
...
{
	int ret;
	gnutls_datum_t *d_cert = NULL, *d_key = NULL, *d_cacert = NULL;
        rb_ssl_ctx *sctx;           
	...
	if(cacert != NULL)
	{
		if((d_cacert = rb_load_file_into_datum_t(cacert)) == NULL )
		{
			rb_lib_log("rb_setup_ssl_server: Error loading CA certificae: %s", strerror(errno));
			goto cleanup;
		}
	...
	if(gnutls_certificate_allocate_credentials(&sctx->x509) != GNUTLS_E_SUCCESS)
	{
	...
	if((ret =
	    gnutls_certificate_set_x509_key_mem(sctx->x509, d_cert, d_key,
	...
	if(d_cacert != NULL)
		rb_free_datum_t(d_cacert);
	
	if(dhfile != NULL)
	{
		if(gnutls_dh_params_init(&sctx->dh_params) == GNUTLS_E_SUCCESS)
		{
			gnutls_datum_t *data;
			int xret;
			...
			if(data != NULL)
			{
				xret = gnutls_dh_params_import_pkcs3(sctx->dh_params, data,
								     GNUTLS_X509_FMT_PEM);
				if(xret < 0)
					rb_lib_log
			...
			gnutls_certificate_set_dh_params(sctx->x509, sctx->dh_params);
		}
		else
	...
	if(ssl_cipher_list != NULL)
		sctx->ssl_cipher_list = rb_strdup(ssl_cipher_list);

	sctx->tls_min_ver = tls_min_ver;
	return sctx;

cleanup_x509:
	gnutls_certificate_free_credentials(sctx->x509);
cleanup_datum_t:
	if(d_cert != NULL)
		rb_free_datum_t(d_cert);
	if(d_cacert != NULL)
...
{
	int ret;
	ret = rb_listen(F, backlog, defer_accept);
...
rb_ssl_start_connected(rb_fde_t *F, CNCB * callback, void *data, int timeout)
{
	...
	if(F->sctx->x509 != NULL)
		gnutls_credentials_set(SSL_P(F), GNUTLS_CRD_CERTIFICATE, F->sctx->x509);
...
{
	return gnutls_strerror(F->sslerr.ssl_errno);
}
...
rb_ssl_ctx * rb_setup_ssl_client(const char *ssl_cipher_list, const char *cert, const char *keyfile)
{
	...
	sctx->tls_min_ver = RB_TLS_VER_TLS1;
	return sctx;
...
rb_ssl_get_certfp(rb_fde_t *F, uint8_t certfp[RB_SSL_CERTFP_LEN])
{
	...
	if (gnutls_x509_crt_get_fingerprint(cert, GNUTLS_DIG_SHA256, digest, &digest_size) < 0)
	{
		gnutls_x509_crt_deinit(cert);
		return 0;
	}
