...
rb_init_netio_epoll(void)
{
	...
	ep_info->pfd = rb_malloc(sizeof(struct epoll_event) * (unsigned long)ep_info->pfd_size);
...
rb_setselect_epoll(rb_fde_t *F, unsigned int type, PF * handler, void *client_data)
{
	...
	ep_event.events = (uint32_t)F->pflags;
	ep_event.data.ptr = F;
...
rb_select_epoll(long delay)
{
	...
	num = epoll_wait(ep_info->ep, ep_info->pfd, ep_info->pfd_size, (int)delay);
	...
	for(i = 0; i < num; i++)
	{
		...
		if(old_flags != flags)
		{
			if(flags == 0)
				...
				op = EPOLL_CTL_MOD;
			F->pflags = flags; 
			ep_event.events = (uint32_t)flags;
			ep_event.data.ptr = F;
...
#if SIZEOF_STRUCT_SIGNALFD_SIGINFO >= 128
#define librb_signalfd_siginfo signalfd_siginfo
#else
/* bleh..work around a glibc header bug on 32bit systems */
struct librb_signalfd_siginfo
{
	uint32_t ssi_signo;
	int32_t ssi_errno;
	int32_t ssi_code;
...
{
	static struct librb_signalfd_siginfo fdsig[SIGFDIOV_COUNT];
	static struct iovec iov[SIGFDIOV_COUNT];
	rb_ev_entry *ev;
	ssize_t ret;

	for(int x = 0; x < SIGFDIOV_COUNT; x++)
	{
		iov[x].iov_base = &fdsig[x];
		iov[x].iov_len = sizeof(struct librb_signalfd_siginfo);
	}
	...
	while(1)
	{
		...
		for(int x = 0; x < ret / (int)sizeof(struct librb_signalfd_siginfo); x++)
		{
#if __WORDSIZE == 32 && defined(__sparc__)
			uint32_t *q = (uint32_t *)&fdsig[x].ssi_ptr;
			ev = (rb_ev_entry *)q[0];
#else
			ev = (rb_ev_entry *)(uintptr_t)(fdsig[x].ssi_ptr);
...
static int
rb_epoll_sched_event_signalfd(rb_ev_entry *event, time_t when)
{
	timer_t *id;
	struct sigevent ev;
...
{
	rb_ev_entry *event = (rb_ev_entry *)data;
	ssize_t retlen;
	uint64_t count;
...
static int
rb_epoll_sched_event_timerfd(rb_ev_entry *event, time_t when)
{
	struct itimerspec ts;
	char buf[FD_DESC_SZ + 8];
	int fd;
	rb_fde_t *F;
...
int
rb_epoll_sched_event(rb_ev_entry *event, time_t when)
{
#ifdef USE_TIMERFD_CREATE
	if(can_do_timerfd)
	{
...
void
rb_epoll_unsched_event(rb_ev_entry *event)
{
#ifdef USE_TIMERFD_CREATE
	if(can_do_timerfd)
	{
