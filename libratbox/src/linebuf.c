...
void
rb_linebuf_init(void)
{
	/* nothing to do without balloc now */
	return;
}

rb_buf_head_t *
rb_linebuf_bufhead_alloc(void)
{
	return rb_malloc(sizeof(rb_buf_head_t));
}

void 
rb_linebuf_bufhead_free(rb_buf_head_t *bufhead)
{
	rb_free(bufhead);
}

static rb_buf_line_t *
rb_linebuf_allocate(void)
{
	return rb_malloc(sizeof(rb_buf_line_t));
}

static void
rb_linebuf_free(rb_buf_line_t * p)
{
	rb_free(p);
}
 ...
 */
static rb_buf_line_t *
rb_linebuf_new_line(rb_buf_head_t * bufhead)
{
 ...
 */
static void
rb_linebuf_done_line(rb_buf_head_t * bufhead, rb_buf_line_t * bufline, rb_dlink_node *node)
{
...
rb_linebuf_skip_crlf(char *ch, size_t len)
{
	size_t orig_len = len;
 ...
 */
void
rb_linebuf_newbuf(rb_buf_head_t * bufhead)
{
	/* not much to do right now :) */
	memset(bufhead, 0, sizeof(rb_buf_head_t));
}
 ...
 */
void
rb_linebuf_donebuf(rb_buf_head_t * bufhead)
{
	while(bufhead->list.head != NULL)
	{
		rb_linebuf_done_line(bufhead, (rb_buf_line_t *) bufhead->list.head->data,
				     bufhead->list.head);
	}
}
 ...
 */
static ssize_t
rb_linebuf_copy_line(rb_buf_head_t * bufhead, rb_buf_line_t * bufline, char *data, size_t len)
{
	size_t cpylen = 0;	/* how many bytes we've copied */
	char *ch = data;	/* Pointer to where we are in the read data */
	char *bufch = bufline->buf + bufline->len;
	ssize_t clen = 0;		/* how many bytes we've processed,
	...
	if(bufline->terminated == true)
		return 0;

	clen = rb_linebuf_skip_crlf(ch, len);
	cpylen = (size_t)clen;
	if(clen == -1)
		return -1;
 ...
 */
static ssize_t
rb_linebuf_copy_raw(rb_buf_head_t * bufhead, rb_buf_line_t * bufline, char *data, size_t len)
{
	size_t cpylen = 0;		/* how many bytes we've copied */
	char *ch = data;	/* Pointer to where we are in the read data */
	char *bufch = bufline->buf + bufline->len;
 ...
 */
int
rb_linebuf_parse(rb_buf_head_t * bufhead, char *data, size_t len, bool raw)
{
	rb_buf_line_t *bufline;
	ssize_t cpylen;
	int linecnt = 0;
...
rb_linebuf_get(rb_buf_head_t * bufhead, char *buf, size_t buflen, bool partial, bool raw)
{
	...
	if(!(partial || bufline->terminated == true))
 ...
 * rb_linebuf_attach
 *
 * attach the lines in a rb_buf_head_t to another rb_buf_head_t
 * without copying the data (using refcounts).
 */
void
rb_linebuf_attach(rb_buf_head_t * bufhead, rb_buf_head_t * new)
{
	rb_dlink_node *ptr;
	rb_buf_line_t *line;
 ...
 */
void
rb_linebuf_putmsg(rb_buf_head_t * bufhead, const char *format, va_list * va_args,
		  const char *prefixfmt, ...)
{
	...
	if(prefixfmt != NULL)
	{
		...
		len = (size_t)vsnprintf(bufline->buf, BUF_DATA_SIZE, prefixfmt, prefix_args);
		va_end(prefix_args);
	}

	if(va_args != NULL)
	{
		len += (size_t)vsnprintf((bufline->buf + len), (BUF_DATA_SIZE - len), format, *va_args);
	}
...
void
rb_linebuf_putbuf(rb_buf_head_t * bufhead, const char *buffer)
{
	rb_buf_line_t *bufline;
	size_t len = 0;
...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"     
void
rb_linebuf_put(rb_buf_head_t * bufhead, const char *format, ...)
{
	rb_buf_line_t *bufline;
	size_t len = 0;
	va_list args;
	...
	if(rb_unlikely(format != NULL))
	{
		...
		len = (size_t)vsnprintf(bufline->buf, BUF_DATA_SIZE, format, args);
		va_end(args);
	}
 ...
 */
ssize_t
rb_linebuf_flush(rb_fde_t *F, rb_buf_head_t * bufhead)
{
	rb_buf_line_t *bufline;
	ssize_t retval;
 ...
 */
	...
	if(!rb_fd_ssl(F))
	{
		...
		xret = retval = (ssize_t)rb_writev(F, vec, x);
		if(retval <= 0)
			return retval;
		...
		for(y = 0; y < x; y++)
		{
			...
			if(xret >= (ssize_t)(bufline->len - bufhead->writeofs))
			{
...
size_t rb_linebuf_len(rb_buf_head_t *bufhead)
{
	return bufhead->len;
}

size_t rb_linebuf_alloclen(rb_buf_head_t *bufhead)
{
	return bufhead->alloclen;
}

unsigned int rb_linebuf_numlines(rb_buf_head_t *bufhead)
{
