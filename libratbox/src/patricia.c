...
prefix_toa2x(rb_prefix_t *prefix, char *buf, int buf_len, int with_len)
{
	...
	rb_inet_ntop(prefix->family, &prefix->add.sin, buf, buf_len);
	if(with_len)
	{
...
rb_patricia_node_t *
rb_make_and_lookup_ip(rb_patricia_tree_t *tree, struct sockaddr *in, int bitlen)
{
	rb_prefix_t *prefix;
	rb_patricia_node_t *node;
	void *ipptr = NULL;
...
rb_patricia_node_t *
rb_make_and_lookup(rb_patricia_tree_t *tree, const char *string)
{
	rb_prefix_t *prefix;
	rb_patricia_node_t *node;
