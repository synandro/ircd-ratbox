...
struct _pollfd_list
{
	...
	nfds_t maxindex;	/* highest FD number */
...
{
	int fd;
	pollfd_list.pollfds = rb_malloc((unsigned long)rb_getmaxconnect() * (sizeof(struct pollfd)));
	...
	if(rb_unlikely(fd >= pollfd_list.allocated))
	{
		...
		pollfd_list.pollfds =
			rb_realloc(pollfd_list.pollfds,
				   (unsigned long)pollfd_list.allocated * (sizeof(struct pollfd)));
...
rb_setselect_poll(rb_fde_t *F, unsigned int type, PF * handler, void *client_data)
{
	...
	if(F->pflags <= 0)
	{
		...
		if(pollfd_list.maxindex == (nfds_t)F->fd)
		{
	...
	else
	{
		...
		if((nfds_t)F->fd > pollfd_list.maxindex)
			pollfd_list.maxindex = (nfds_t)F->fd;
	}
