...
static rb_ev_entry *rb_timeout_ev;


static const char *rb_err_str[] = {
	[RB_OK] = "Comm OK",
	[RB_ERR_BIND] = "Error during bind()",
	[RB_ERR_DNS] = "Error during DNS lookup",
	[RB_ERR_TIMEOUT] = "connect timeout",
	[RB_ERR_CONNECT] = "Error during connect()",
	[RB_ERROR] = "I/O Loop Error",
	[RB_ERROR_SSL] = "SSL Error",
	[RB_ERR_MAX] = NULL
};
...
rb_accept_tryaccept(rb_fde_t *F, void *data)
{
	...
	while(1)
	{
		...
		if(F->type & RB_FD_SSL)
		{
			rb_ssl_accept_setup(F, new_F, (struct sockaddr *)&st, (rb_socklen_t)addrlen);
		}
		else
#endif /* HAVE_SSL */
		{
			F->accept->callback(new_F, RB_OK, (struct sockaddr *)&st, (rb_socklen_t)addrlen,
					    F->accept->data);
		}
	}
...
int
rb_connect_sockaddr(rb_fde_t *F, struct sockaddr *addr, rb_socklen_t len)
{
	if(F == NULL)
		return 0;
 ...
 */
int
rb_listen(rb_fde_t *F, int backlog, bool defer_accept)
{
	int result;
	...
	if(defer_accept == true && result == 0)
	{
		...
		setsockopt(F->fd, SOL_SOCKET, SO_ACCEPTFILTER, &afa, sizeof(afa));
	}
#endif
	return result;
}
...
inet_pton4(const char *src, unsigned char *dst)
{
	...
	*(tp = tmp) = 0;
	while((ch = *src++) != '\0')
	{
		...
		if(ch >= '0' && ch <= '9')
		{
 			unsigned int new =  (unsigned int)(*tp * 10 + (ch - '0'));
...
inet_ntop4(const unsigned char *src, char *dst, unsigned int size)
{
	if(size < 16)
		return NULL;
	snprintf(dst, size, "%u.%u.%u.%u", src[0], src[1], src[2], src[3]);
	return dst;
}
...
inet_ntop6(const unsigned char *src, char *dst, unsigned int size)
{
	...
	for(i = 0; i < IN6ADDRSZ; i += 2)
		words[i / 2] = (unsigned int)((src[i] << 8) | src[i + 1]);
	best.base = -1;
	best.len = 0;
	cur.base = -1;
	cur.len = 0;
	...
	for(i = 0; i < (IN6ADDRSZ / INT16SZ); i++)
	{
		...
		{
			if(!inet_ntop4(src + 12, tp, sizeof(tmp) - (unsigned int)(tp - tmp)))
				return (NULL);
			tp += strlen(tp);
			break;
		}
...
const char *
rb_inet_ntop(int af, void *src, char *dst, rb_socklen_t len)
{
	switch(af)
	{
		case AF_INET:
...
const char *
rb_inet_ntop_sock(struct sockaddr *src, char *dst, rb_socklen_t size)
{
...
static int (*io_sched_event) (rb_ev_entry *, time_t);
static void (*io_unsched_event) (rb_ev_entry *);
...
int
rb_io_sched_event(rb_ev_entry *ev, time_t when)
{
...
void
rb_io_unsched_event(rb_ev_entry *ev)
{
...
rb_recv_fd_buf(rb_fde_t *F, void *data, size_t datasize, rb_fde_t **xF, unsigned int nfds)
{
	...
	{
		unsigned int rfds = (unsigned int)((size_t)((unsigned char *)cmsg + cmsg->cmsg_len - CMSG_DATA(cmsg))) / sizeof(int);

		for(unsigned int x = 0; x < nfds && x < rfds; x++)
		{
...
ssize_t
rb_send_fd_buf(rb_fde_t *xF, rb_fde_t **F, unsigned int count, void *data, size_t datasize, pid_t pid)
{
	struct msghdr msg;
	struct cmsghdr *cmsg;
	struct iovec iov[1];
	...
	if(count > 0)
	{
		size_t len = CMSG_SPACE(sizeof(int) * count);
		char buf[len];
