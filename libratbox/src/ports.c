...
rb_select_ports(long delay)
{
	...
	for(i = 0; i < nget; i++)
	{
		if(pelst[i].portev_source == PORT_SOURCE_FD)
		{
			...
			if((pelst[i].portev_events & (POLLIN | POLLHUP | POLLERR))
			   && (hdl = F->read_handler))
			{
				F->read_handler = NULL;
				hdl(F, F->read_data);
			}
			if((pelst[i].portev_events & (POLLOUT | POLLHUP | POLLERR))
			   && (hdl = F->write_handler))
			{
		...
		{
			ev = (rb_ev_entry *)pelst[i].portev_user;
			rb_run_event(ev);
		}
	}
	return RB_OK;
...
int
rb_ports_sched_event(rb_ev_entry *event, time_t when)
{
	timer_t *id;
	struct sigevent ev;
	port_notify_t not;
...
void
rb_ports_unsched_event(rb_ev_entry *event)
{
