...
#ifdef __GNUC__ 
void rb_lib_log(const char *, ...) __attribute((format(printf, 1, 2)));
void rb_lib_restart(const char *, ...) __attribute((format(printf, 1, 2)));
void rb_lib_die(const char *, ...) __attribute((format(printf, 1, 2)));
#else
...
#ifdef __GNUC__
void rb_lib_loop(long delay) __attribute((__noreturn__));
#else
...
extern const char *libratbox_creation;
extern const char *libratbox_infotext[];


#if RB_SIZEOF_TIME_T == 4
#define LIBRB_TT_FMT "d"
#else
#if RB_SIZEOF_TIME_T > 4 && RB_SIZEOF_LONG == RB_SIZEOF_TIME_T
