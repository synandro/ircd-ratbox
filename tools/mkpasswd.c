...
main(int argc, char *argv[])
{
	...
	{
		plaintext = rb_strdup(getpass("Password: "));
		repeat_plaintext = getpass("Repeat Password: ");
		if(strcmp(plaintext, repeat_plaintext) != 0)
		{
...
make_md5_salt_para(char *saltpara)
{
	...
	fprintf(stderr, "Invalid Salt, please use up to 16 random alphanumeric characters\n");
	exit(1);
}
...
make_md5_salt(int length)
{
	...
	if(length > 16)
	{
		fprintf(stderr, "MD5 salt length too long\n");
		exit(0);
	}
	salt[0] = '$';
	salt[1] = '1';
...
make_sha256_salt(int length)
{
	...
	if(length > 16)
	{
		fprintf(stderr, "SHA256 salt length too long\n");
		exit(1);
	}
	salt[0] = '$';
	salt[1] = '5';
...
make_sha512_salt(int length)
{
	...
	if(length > 16)
	{
		fprintf(stderr, "SHA512 salt length too long\n");
		exit(1);
	}
	salt[0] = '$';
	salt[1] = '6';
...
make_bf_salt_para(int rounds, char *saltpara)
{
	...
	fprintf(stderr, "Invalid Salt, please use up to 22 random alphanumeric characters\n");
	exit(1);
}
...
void
full_usage()
{	fprintf(stderr, "mkpasswd [-m|-d|-b|-e] [-l saltlength] [-r rounds] [-s salt] [-p plaintext]\n");
	fprintf(stderr, "-x Generate a SHA256 password\n");
	fprintf(stderr, "-y Generate a SHA512 password\n");
	fprintf(stderr, "-m Generate an MD5 password\n");
	fprintf(stderr, "-d Generate a DES password\n");
	fprintf(stderr, "-b Generate a BlowFish password\n");
	fprintf(stderr, "-e Generate an Extended DES password\n");
	fprintf(stderr, "-l Specify a length for a random MD5 or BlowFish salt\n");
	fprintf(stderr, "-r Specify a number of rounds for a BlowFish or Extended DES password\n");
	fprintf(stderr, "   BlowFish:  default 4, no more than 6 recommended\n");
	fprintf(stderr, "   Extended DES:  default 25\n");
	fprintf(stderr, "-s Specify a salt, 2 alphanumeric characters for DES, up to 16 for MD5,\n");
	fprintf(stderr, "   up to 22 for BlowFish, and 4 for Extended DES\n");
	fprintf(stderr, "-p Specify a plaintext password to use\n");
	fprintf(stderr, "Example: mkpasswd -m -s 3dr -p test\n");
	exit(1);
}
...
brief_usage()
{
	fprintf(stderr, "mkpasswd - password hash generator\n");
	fprintf(stderr, "Standard DES:  mkpasswd [-d] [-s salt] [-p plaintext]\n");
	fprintf(stderr, "Extended DES:  mkpasswd -e [-r rounds] [-s salt] [-p plaintext]\n");
	fprintf(stderr, "         MD5:  mkpasswd -m [-l saltlength] [-s salt] [-p plaintext]\n");
	fprintf(stderr, "    BlowFish:  mkpasswd -b [-r rounds] [-l saltlength] [-s salt]\n");
	fprintf(stderr, "                           [-p plaintext]\n");
	fprintf(stderr, "Use -h for full usage\n");
	exit(1);
}
